...
export default function AccessManagement() {
  ...
  const { showNotification, user, files, fetching } = useContext(AuthContext);
  ...
  const fetchAccessList = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await API.get(`/api/files/access-list/${user.address}`);
  
      if (fetching) return;
  
      const filesWithAccessList = files.map((file) => {
        const accessInfo = data.filesWithAccessList.find(
          (entry) => entry.fileId === String(file.fileId)
        );
      ...
      setFilesWithAccess(filesWithAccessList);
    } catch (error) {
      console.error(error);
      showNotification("Failed to fetch access lists.", "error");
    } finally {
      setLoading(false);
    }
  }, [files, user.address, fetching, API, setFilesWithAccess, setLoading, showNotification]);
 
  useEffect(() => {
    fetchAccessList();
  }, [user,files,fetching,fetchAccessList]);
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-800">
                  <th className="text-left py-3 px-4 font-medium text-gray-400">File ID</th>
                  ...
                  <th className="text-left py-3 px-4 font-medium text-gray-400">Access List</th>
                </tr>
              </thead>
              <tbody>
                {filesWithAccess.length > 0 ? (
                  filesWithAccess.map((file) => (
                    <tr key={file.fileId} className="border-b border-gray-800 hover:bg-gray-800/50 transition-colors">
                      <td className="py-3 px-4 text-gray-300">{file.fileId}</td>
                        ...
                        <div className="max-h-28 overflow-y-auto pr-2 space-y-1">
                          {(file.accessList && file.accessList.length > 0) && (
                            file.accessList.map((address, index) => (
                              <div key={index} className="flex items-center justify-between text-sm bg-gray-900 px-2 py-1 rounded">
                                <div className="flex items-center gap-2">
                                ...
                                <button
                                  onClick={() => handleRevokeAccess(file.fileId, address)}
