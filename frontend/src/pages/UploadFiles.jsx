...
import { File, FileText, Image, Music, Upload, Video, X, RefreshCw, Slice} from "lucide-react"
...
const uploadToBackend = async () => {
  try {
    ...
    setSelectedFiles(null);
      ...
      showNotification(`File "${selectedFiles.name}" successfully uploaded to IPFS`);
                ...
                {selectedFiles &&
                  <div className="flex items-center p-4 bg-gray-800/50 rounded-lg">
                    <div className="w-10 h-10 rounded-lg bg-gray-800 flex items-center justify-center mr-4">
                      {selectedFiles.type === "image" && <Image className="w-5 h-5 text-blue-400" />}
                      {selectedFiles.type === "document" && <FileText className="w-5 h-5 text-purple-400" />}
                      {selectedFiles.type === "video" && <Video className="w-5 h-5 text-green-400" />}
                      {selectedFiles.type === "audio" && <Music className="w-5 h-5 text-yellow-400" />}
                      {selectedFiles.type === "archive" && <File className="w-5 h-5 text-gray-400" />}
                      {selectedFiles.type === "file" && <File className="w-5 h-5 text-gray-400" />}
                    </div>
                    <div className="flex-1 min-w-0 mr-4">
                      <h3 className="text-sm font-medium truncate">{selectedFiles.name}</h3>
                      <p className="text-xs text-gray-400">{selectedFiles.size}</p>
                    </div>
